...
## [2026-07-21] 홈 화면 게스트에게 로그인 버튼 추가
- 수정 파일: `src/app/(main)/home/page.tsx`
- 변경 내용: "안녕하세요, 게스트 님!" 인사말 우측에 "로그인 >" 버튼 추가(게스트에게만 노출, 회원은 미노출). 클릭 시 `ROUTES.login`(`/`, 로그인 초기화면)으로 이동.

## [2026-07-21] GuestSaveProgressCard 반응형 추가 조정 — 실제 원인은 record 페이지의 하드코딩된 paddingBlock 오버라이드
- 수정 파일: `src/components/auth/GuestSaveProgressCard.tsx`, `src/app/(main)/record/page.tsx`
- 변경 내용:
  - 카드 좌우 두 블록(문구+버튼 / 뱃지) 사이 `gap`을 `clamp(14px, 5cqw, 16px)`로 조정(최솟값을 8→14로 올려 320px에서도 답답하지 않게).
  - "Kakao로 시작하기" 버튼 내부 패딩·화살표 아이콘과의 간격을 더 공격적으로 축소(`gap: clamp(3px, 2.5cqw, 16px)`, `paddingInline: clamp(6px, 2.5cqw, 13px)`).
  - **근본 원인 발견**: `record/page.tsx`에서 `GuestSaveProgressCard`에 `style={{ paddingBlock: "55px" }}`를 고정값으로 넘기고 있었는데, 컴포넌트 내부에서 `...style`이 자체 반응형 기본값보다 뒤에 스프레드되어 항상 이 고정값이 이겨버림 — 그래서 앞서 아무리 반응형으로 고쳐도 세로 패딩이 전혀 안 줄어들고 있었음(사용자가 "왜 안 바뀌냐"고 재차 물어본 원인). 컴포넌트의 `paddingBlock` 기본값을 `clamp(24px, 14cqw, 55px)`로 바꾸고 record 페이지의 고정 오버라이드는 제거.
- 비고: 처음엔 좁은 화면에서 세로(flex-col) 배치로 바꾸는 시도를 했었으나 "원래 디자인(가로 배치)은 유지하고 크기만 반응형으로 하라"는 피드백을 받아 구조는 그대로 두고 크기만 조정하는 방향으로 되돌림.

## [2026-07-21] 긴급 신고 안내 "바로 전화 하기" 카드 간격 반응형화
- 수정 파일: `src/app/(main)/emergency/page.tsx`
- 변경 내용: 경찰청(112)/금융감독원(1332) 카드 사이 간격(`gap-3.75` 고정)을 `clamp(8px, 3.6cqw, 15px)`로 반응형화.

## [2026-07-21] 게스트 로그인 유도 카드(GuestSaveProgressCard) 반응형화 + 문구 커스터마이즈
- 수정 파일: `src/components/auth/GuestSaveProgressCard.tsx`, `src/app/(main)/record/page.tsx`
- 변경 내용:
  - 학습 기록 화면 진입 시 게스트에게 보이는 문구를 "이번 학습을 저장해보세요!"에서 "학습기록을 확인해보세요!"로 변경 — `message` prop을 추가해 화면별로 다른 문구를 넘길 수 있도록 함(마무리 퀴즈 화면은 기존 문구 유지).
  - 320px처럼 좁은 화면에서 문구가 뱃지 영역과 겹치던 문제 수정. 처음엔 좁은 화면에서 세로 배치로 바꾸는 방식으로 시도했으나, "원래 디자인(가로 배치)은 그대로 두고 반응형만 하라"는 피드백에 따라 **레이아웃 구조는 그대로 유지**하고 카드 패딩·간격·"Kakao로 시작하기" 버튼의 패딩/간격만 `clamp()`로 좁아지도록 수정 — 기본(넓은) 폭에서는 기존 디자인과 동일하게 보이고, 좁은 폭에서만 자연스럽게 줄어듦.

## [2026-07-21] 로그인 화면 데스크톱 좌측 패널 반응형화 (노트북 폭에서 깨지던 문제)
- 수정 파일: `src/app/layout.tsx`
- 변경 내용: `min-[1400px]:`에서만 보이는 좌측 마케팅 패널(`<aside>`)의 내부 요소(문구, 별 이미지, 로고, 좌측 패딩)가 전부 `vw` 단위로 크기가 정해져 있었는데, 패널 자체의 실제 너비는 `right: 888px`(고정값)로 정해져 있어 전체 뷰포트 너비와 패널의 실제 너비가 비례하지 않았음 — 노트북처럼 1400px 언저리의 좁은 폭에서는 패널 자체가 확 좁아지는데 내부 요소는 여전히 전체 화면 기준 vw로 크게 잡혀서 문구가 밀려 들여쓰기되고 로고가 우측으로 밀리는 것처럼 보이는 문제였음. `<aside>`에 `@container`를 추가하고 내부 요소를 패널 자체 너비 기준 `cqw`로 전환, 우측 폰 프레임과의 간격(`mr-97` 고정값)도 `clamp(160px, 14vw, 388px)`로 반응형화(처음엔 최솟값을 40px로 낮게 잡아 프레임이 화면 오른쪽 끝에 너무 붙어보여서 160px로 올림). 로고 이미지도 width/height를 각각 다른 vw 계수로 clamp하고 있어 뷰포트에 따라 종횡비가 미묘하게 뒤틀리는 부수적 버그가 있었음 — 로고는 `height: "auto"`로 바꿔 원본 종횡비를 유지하도록 수정.
- 비고: Playwright로 1400/1440/1536/1920/2560px 구간에서 패딩 비율·로고 폭·종횡비가 자연스럽게 이어지는 것을 확인.

## [2026-07-21] 시나리오 상세 화면의 기관사칭 아이콘도 학습하기 목록과 같은 크기로 통일
- 수정 파일: `src/app/(main)/learn/[caseId]/page.tsx`
- 변경 내용: 시나리오 상세(`학습 시나리오`) 헤더의 고정 20px 배지도 `ScenarioCard`와 동일한 구조인데 여전히 상한 19px짜리 `INSTITUTION_ICON_SIZE`를 쓰고 있었음 — `INSTITUTION_ICON_SIZE_LEARN`(상한 14px)으로 교체해 학습하기 목록과 크기를 통일.
